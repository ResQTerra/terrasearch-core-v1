...
class MessageHandler(private val myPeerID: String) {
    ...
    private val lidarDataReassembler = com.bitchat.android.lidar.LidarDataReassembler()
    ...
    suspend fun handleLidarPacket(routed: RoutedPacket) {
        val packet = routed.packet
        val lidarPacket = when(MessageType.fromValue(packet.type)) {
            MessageType.LIDAR_FRAGMENT_START -> {
                val buffer = ByteBuffer.wrap(packet.payload)
                val scanTimestampSec = buffer.long
                val scanTimestampNanos = buffer.int
                val totalFragments = buffer.int
                com.bitchat.android.model.LidarFragmentStart(scanTimestampSec, scanTimestampNanos, totalFragments)
            }
            MessageType.LIDAR_FRAGMENT_CONTINUE -> {
                val buffer = ByteBuffer.wrap(packet.payload)
                val scanTimestampSec = buffer.long
                val scanTimestampNanos = buffer.int
                val fragmentSequence = buffer.int
                val data = ByteArray(buffer.remaining())
                buffer.get(data)
                com.bitchat.android.model.LidarFragmentContinue(scanTimestampSec, scanTimestampNanos, fragmentSequence, data)
            }
            MessageType.LIDAR_FRAGMENT_END -> {
                val buffer = ByteBuffer.wrap(packet.payload)
                val scanTimestampSec = buffer.long
                val scanTimestampNanos = buffer.int
                val fragmentSequence = buffer.int
                val data = ByteArray(buffer.remaining())
                buffer.get(data)
                com.bitchat.android.model.LidarFragmentEnd(scanTimestampSec, scanTimestampNanos, fragmentSequence, data)
            }
            else -> null
        }

        if (lidarPacket != null) {
            val scan = lidarDataReassembler.processPacket(lidarPacket)
            if (scan != null) {
