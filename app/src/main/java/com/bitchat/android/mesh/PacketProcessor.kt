...
class PacketProcessor(private val myPeerID: String) {
    ...
    private suspend fun handleReceivedPacket(routed: RoutedPacket) {
        ...
        when (messageType) {
            ...
            MessageType.LIDAR_FRAGMENT_START, MessageType.LIDAR_FRAGMENT_CONTINUE, MessageType.LIDAR_FRAGMENT_END -> handleLidarPacket(routed)
            else -> {
    ...
    private suspend fun handleLidarPacket(routed: RoutedPacket) {
        val peerID = routed.peerID ?: "unknown"
        Log.d(TAG, "Processing LIDAR packet from ${formatPeerForLog(peerID)}")
        delegate?.handleLidarPacket(routed)
    }
...
interface PacketProcessorDelegate {
    ...
    fun handleLidarPacket(routed: RoutedPacket)
